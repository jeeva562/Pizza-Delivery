      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex items-start justify-between gap-3 sm:gap-4">
          ...
          <div className="flex-1 max-w-[160px] sm:max-w-md">
            <div className="glass rounded-lg px-3 sm:px-4 py-2 sm:py-3">
              <div className="flex items-center justify-between mb-1.5 sm:mb-2">
                <div className="flex items-center gap-1.5 sm:gap-2">
                  <MapPin className="w-4 h-4 sm:w-4 sm:h-4" style={{ color: levelColor }} />
                  <span className="font-display text-xs sm:text-sm font-semibold" style={{ color: levelColor }}>
                    {level + 1}/{totalLevels}
                  </span>
                </div>
                <span className="text-[11px] sm:text-xs text-muted-foreground truncate max-w-[70px] sm:max-w-none">
                  {levelName}
                </span>
              </div>

              {/* Progress bar */}
              <div className="h-2 sm:h-2 bg-muted rounded-full overflow-hidden">
                <div
              ...
              <div className="flex justify-center gap-0.5 mt-1.5 sm:hidden">
                {Array.from({ length: totalLevels }).map((_, i) => (
                  <div
                    key={i}
                    className={`w-1.5 h-1.5 rounded-full ${i <= level ? "opacity-100" : "opacity-30"}`}
          ...
          <div className="flex flex-col gap-2 sm:gap-3 items-end">
            {/* Fuel */}
            <div className="glass rounded-lg px-3 sm:px-4 py-1.5 sm:py-2">
              <div className="flex items-center gap-2 sm:gap-2 mb-1">
                <Zap className={`w-5 h-5 sm:w-4 sm:h-4 ${fuel > 30 ? "text-primary" : "text-orange-500"}`} />
                <span className="text-xs sm:text-xs text-muted-foreground font-mono font-semibold">{Math.round(fuel)}%</span>
              </div>
              <div className="w-16 sm:w-20 h-1.5 sm:h-1.5 bg-muted rounded-full overflow-hidden">
                <div
